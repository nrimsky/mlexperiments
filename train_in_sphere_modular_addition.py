...
from mlp_modular import MLP, test_model, get_train_test_loaders, plot_embeddings, plot_embeddings_chunks
import torch as t
...
def hessian_eig(model, loss_fn, train_data_loader, num_batches=30, device="cuda", n_top_vectors=200, param_extract_fn=None):
...
def train_in_sphere(model, dataloader, top_eigenvectors, radius = 1, lambda_sphere = 10, lambda_orth = .1, lr = 1e-3, n_epochs = 3, device = 'cuda', lr_decay=0.999):
    model.to(device)
    ...
    for epoch in range(n_epochs):
        ...
        scheduler.step()
        if epoch % 50 == 0:
            print(f"Epoch {epoch}/{n_epochs}, avg_sphere_reg = {tot_sphere_reg/idx}, avg_orth_reg = {tot_orth_reg/idx}, avg_ce_loss = {tot_ce_loss/idx}")
            idx = 0
            tot_sphere_reg = 0
            tot_orth_reg = 0
            tot_ce_loss = 0
...
def main():
    ...
    model.eval()
    loss_fn = t.nn.CrossEntropyLoss()
    train_loader, test_loader = get_train_test_loaders(train_frac=0.4, batch_size=256, vocab_size=VOCAB_SIZE)
    _, _, eigenvectors = hessian_eig(model, loss_fn, test_loader, device="cuda", n_top_vectors=N_EIGENVECTORS, param_extract_fn=get_module_parameters)
    train_in_sphere(model, train_loader, eigenvectors, radius=RADIUS, lambda_sphere=LAMBDA_SPHERE, lambda_orth=LAMBDA_ORTH, lr=LR, n_epochs=N_EPOCHS, device="cuda", lr_decay=LR_DECAY)
